...
class HctSolver {
    ...
    static let SCALED_DISCOUNT_FROM_LINRGB: [[Double]] = [
        [0.001200833568784504, 0.002389694492170889, 0.0002795742885861124],
        [0.0005891086651375999, 0.0029785502573438758, 0.0003270666104008398],
        [0.00010146692491640572, 0.0005364214359186694, 0.0032979401770712076],
    ]

    static let LINRGB_FROM_SCALED_DISCOUNT: [[Double]] = [
        [1373.2198709594231, -1100.4251190754821, -7.278681089101213],
        [-271.815969077903, 559.6580465940733, -32.46047482791194],
        [1.9622899599665666, -57.173814538844006, 308.7233197812385],
    ]

    static let Y_FROM_LINRGB: [Double] = [0.2126, 0.7152, 0.0722]

    static let CRITICAL_PLANES: [Double] =
        [
     ...
     */
    static func trueDelinearized(_ rgbComponent: Double) -> Double {
    ...
    static func chromaticAdaptation(_ component: Double) -> Double {
        let af: Double = pow(abs(component), 0.42)
        return Double(MathUtils.signum(num: component)) * 400.0 * af / (af + 27.13)
    }
     ...
     */
    static func hueOf(_ linrgb: [Double]) -> Double {
        let scaledDiscount = MathUtils.matrixMultiply(linrgb, SCALED_DISCOUNT_FROM_LINRGB)
        let rA = chromaticAdaptation(scaledDiscount[0])
        let gA = chromaticAdaptation(scaledDiscount[1])
        let bA = chromaticAdaptation(scaledDiscount[2])
        // redness-greenness
    ...
    static func lerpPoint(_ source: [Double], _ t: Double, _ target: [Double])->[Double] {
        return [
     ...
     *     B=coordinate
     */
    static func setCoordinate(_ source: [Double], _ coordinate: Double, _ target: [Double], _ axis: Int)->[Double] {
        let t = intercept(source[axis], coordinate, target[axis])
        return lerpPoint(source, t, target)
    }

    static func isBounded(_ x: Double)->Bool {
     ...
     */
    static func nthVertex(_ y: Double, _ n: Int)->[Double] {
     ...
     */
    static func bisectToSegment(_ y: Double, _ targetHue: Double)->[[Double]] {
    ...
    static func criticalPlaneBelow(_ x: Double)->Int {
        return Int(floor(x - 0.5))
    }

    static func criticalPlaneAbove(_ x: Double)->Int {
     ...
     */
    static func bisectToLimit(_ y: Double, _ targetHue: Double)->[Double] {
            ...
            if left[axis] != right[axis] {
                ...
                for _ in 0..<8 {
                    if abs(rPlane - lPlane) <= 1 {
                        ...
                        let mPlane = Int(floor(Double((lPlane + rPlane)) / 2.0))
    ...
    static func inverseChromaticAdaptation(_ adapted: Double)->Double {
        ...
        return Double(MathUtils.signum(num: adapted)) * pow(base, 1.0 / 0.42)
    }
     ...
     */
    static func findResultByJ(_ hueRadians: Double, _ chroma: Double, _ y: Double)->Int {
        // Initial estimate of j.
        var j = sqrt(y) * 11.0
        // ===========================================================
        // Operations inlined from Cam16 to avoid repeated calculation
        // ===========================================================
        let viewingConditions = ViewingConditions.DEFAULT
        let tInnerCoeff = 1 / pow(1.64 - pow(0.29, viewingConditions.getN()), 0.73)
        let eHue = 0.25 * (cos(hueRadians + 2.0) + 3.8)
        let p1 = eHue * (50000.0 / 13.0) * viewingConditions.getNc() * viewingConditions.getNcb()
        ...
        for iterationRound in 0..<5 {
            ...
            let ac =
                viewingConditions.getAw()
                    * pow(jNormalized, 1.0 / viewingConditions.getC() / viewingConditions.getZ())
            let p2 = ac / viewingConditions.getNbb()
            ...
            let linrgb: [Double] =
                MathUtils.matrixMultiply([ rCScaled, gCScaled, bCScaled ], LINRGB_FROM_SCALED_DISCOUNT)
            // ===========================================================
            // Operations inlined from Cam16 to avoid repeated calculation
            // ===========================================================
     ...
     */
    static func solveToInt(_ hueDegrees: Double, _ chroma: Double, _ lstar: Double)->Int {
     ...
     *     sufficiently close, and chroma will be maximized.
     */
    static func solveToCam(_ hueDegrees: Double, _ chroma: Double, _ lstar: Double)->Cam16 {
